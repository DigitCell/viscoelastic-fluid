...
class Particle {
    ...
    velX: number;
    velY: number;
    springs: { [key: string]: any }; // You can replace 'any' with the actual type if known
...
function moveParticleData(dst: Particle, src: Particle): void {
    dst.posX = src.posX;
    dst.posY = src.posY;
    dst.prevX = src.prevX;
    dst.prevY = src.prevY;
...
class Material {
    ...
    minDistRatio: number;
    linViscosity: number;
    quadViscosity: number;
    maxPressure: number;
    clearScreen:boolean= true;
    svgRender: boolean= false;
    canvasRender: boolean= true; 
...
class Spring {
    ...
    constructor(particleIdxA: number, particleIdxB: number, restLength: number) {
...
class Simulator {
    ...
    numActiveBuckets: number;
    activeBuckets: number[];
    particleListHeads: number[];
    particleListNextIdx: number[];
    material: Material;
    springHash: { [key: string]: any }; // You can replace 'any' with the actual type if known
    ...
    constructor(width: number, height: number, numParticles: number) {
      ...
      this.svgRenderer = new SvgRenderer('#svgoutput', width, height);
      this.canvasRenderer = new CanvasRenderer('simCanvas2D', width, height);
    ...
    applySpringDisplacements(dt:number) {
  ...
  export { Simulator, Particle, Material };